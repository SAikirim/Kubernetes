...
* __디플로이먼트 작성의 흐름__
	...
	6. 레플리카셋 API가 레플리카셋을 etcd에 저장
	7. 레플리카셋 API를 watch하고 있는 ReplicaSet controller가 변화를 감지
	8. ReplicaSet controller가 포드 API를 호출
	9. 포드 API가 포드를 etcd에 저장
	10. 포드 API를 watch하고 있는 Scheduler가 변화를 감지
	11. Scheduler가 어떤 노드에 포드를 배치할지를 판단
	12. Scheduler가 포드 API를 호출하여 etcd 상의 포드 속성(배치 노드)을 갱신
	13. 포드 API를 watch하고 있는 배치 노드의 kubelet이 변화를 감지
	14. kubelet이 컨테이너 런타임(Docker)에 포드 작성을 지시
	15. 각 컴포넌트는 변화에 대비해 계속 watch
