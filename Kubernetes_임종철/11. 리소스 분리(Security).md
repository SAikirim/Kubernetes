...
* 프로젝트별로 클러스터를 분리할 수 있음
	- 다른 프로젝트에게 악영향을 주지 않고, 문제가 생겨도 해당 프로젝틀로 한정됨
...
* 포즈 작성 시에 '서비스 계정'을 명시하지 않으면, 소속도니 네임스페이스의 default 서비스 계정 사용
...
* 서비스 계정은 포드뿐만 아니라 사람이나 툴의 인증과 인가를 사용 가능
	- 서비스 계정을 만들고 필요한 config파일읗 생성, 단말기나 서버에 배포, 작업하는 사람이나 툴이 쿠버네티스 API 조작 가능
...
* 인증이 끝난 계정에 대해 조작할 수 있는 리소스와 가능한 조작을 한정함
	- RBAC(Role Based Access Control)이 대표

---
#### 3. Admission Control
* 인가 플러그인에서 커버할 수 없는 리소르의 특성에 맞춘 제어를 함
...
#### 2. Role과 RoleBinding
* 쿠버네티스 RBAC
	- Role을 만들어 액세스할 수 있는 리소스의종류와 사용할 수 있는 Verb를 모아서 정의
...
* AKS의 API를 통해 인증 정보를 취득하는 명령
	- config(~/.kube/config)에 머지(merge)함
...
* 개발자에세는 AKS클러스터의 토대가 되는 Azure 리소스의 관리자 권한을 부여하지 않는 것을 권장
...
metadata:
  ...
  name: k8sbook-dev
rules:
- apiGroups: ["", "extensions", "apps"]			# 허거할 API Groups
...
/root/Understanding-K8s/chap11 [git::master *] [root@Ubuntu] [20:01]
> az login -u test@kyj9724gmail.onmicrosoft.com --allow-no-subscriptions
Password:
CloudName    IsDefault    Name                       State    TenantId
-----------  -----------  -------------------------  -------  ------------------------------------
AzureCloud   True         N/A(tenant level account)  Enabled  b9dfbc45-d3c2-4324-bbf6-9a4f56e3f506
```
...
* 문제 회피 방법
	- 매니페스트 파일의 작성을 클러스터 관리자가 함
		...
		+ 프로젝트의 자율성이나 주제성, 민첩서이 떨어짐
	- 매티페스트 파일을 투입하기 전에 리뷰를 함
		+ 프로젝트 주체성은 잃지 않음
		+ 리뷰와 투입 타이밍 차이로 클러스터 리소스 이용량이 바뀔수도 있음
	- 클러스터 측에서 상한을 설정
...
*  ResourceQuota를 사용해 총량 제한 가능
	...
	- 이미 움직이소 있는 포드와 작성 요청한 포드의 'Requests' 값을 더하여 ResourceQuota에 정의한 상한을 넘지 않았는지 확인
		+ 넘으면 포드 작성 실패
